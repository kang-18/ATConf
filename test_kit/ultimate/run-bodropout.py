#based on run-test.py
#just for bo with dropout, to deal with copy fill strategy
...
def translate_config_to_numeric(default_config, test_config, os_setting, app_setting):
  # print('show default_config:')
  # print(default_config)

  config=dict(os_setting,**app_setting)

  #default configs, need to transform category into values
  default_config_v={}
  
  if test_config.tune_os is False:
    for key in os_setting.keys():
...
async def main(test_config, init_id, os_setting, app_setting):
  ...
  #to store current (configs,results)
  current_ob_x=[]
  current_ob_y=[]
  task_id = init_id
  ...
  while task_id < test_config.optimizer.iter_limit:
    ...
    if task_id != 0:  # not adding default info, 'cause default cannot convert to numeric form
      ...
      sampled_config_v=translate_config_to_numeric(sampled_config, test_config, os_setting, app_setting)
      ...
      metric_result = mean(metric_results) if len(metric_results) > 0 else .0
      #default configs
      default_config=dict(get_default(os_setting), **get_default(app_setting))
      #default configs into numerics
      default_config_v=translate_config_to_numeric(default_config, test_config, os_setting, app_setting)
...
async def single_test(task_name, task_id, rep, tester, testee, slave1, slave2, slave3, tune_os, clients, _skip=False):
  ...
  try:
    ...
    if tune_os:
      ...
      _print(f'{task_id} - {rep}: resetting os parameters...')
      await run_playbook(
          osconfig_playbook_path,
          host=testee,
          task_name=task_name,
